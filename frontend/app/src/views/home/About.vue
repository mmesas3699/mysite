<template>
  <div class="container-fluid p-0 h-100">
    <HmHeader/>
    <main>
      <div class="container">
        <!-- Title -->
        <div class="row pt-5 pl-sm-5">
          <div class="col-sm">
            <h1 class="font-weight-bold">Miguel Angel Mesa Salazar</h1>
          </div>
        </div>
        <!-- Info -->
        <div class="row pl-sm-5 mt-sm-3 mt-3">
          <div class="col-sm-4">
            <div>
              <p>Soy desarrollador backend autodidacta con buena comunicación oral y escrita, proactivo, con capacidad de análisis, trabajo en equipo y solución de problemas.</p> 
              <p>Tengo dos años de experiencia usando Python y los frameworks para desarrollo web: Flask, Django y Rest Framework.</p>
            </div>
            <br>
            <!-- Contact -->
            <div>
              <h3 class="font-weight-bold">Contacto</h3>
              <p class="contact-info">+57 305 796 1983</p>
              <p class="contact-info">mmesa_@hotmail.com</p>
              <p class="contact-info">Bogotá, Colombia.</p>
            </div>
          </div>
          <!-- Experience / Education -->
          <div class="col-sm">
            <!-- Experience -->
            <div class="mt-sm-0 mt-5">
              <h3 class="font-weight-bold mb-4">Experiencia</h3>
              <!-- Freelancer -->
              <div>
                <h4 class="font-weight-bold">Freelancer</h4>
                <!-- PVD -->
                <div>
                  <span class="text-secondary">2020</span>
                  <ul class="mt-1">
                    <li class="p-1">
                    Desarrollé un API Rest para que un colegio pueda registrar el acceso de los estudiantes que hacen uso de la sala Vive Digital. Permite llevar el tiempo de uso de los equipos y las asignaturas en las que trabajan. <br>
                    El stack usado fue: Django Rest Framework, Docker, Postgresql.
                    </li>
                  </ul>
                </div>
                <!-- Ignacio Cagua -->
                <div>
                  <span class="text-secondary">2019</span>
                  <ul class="mt-1">
                    <li class="p-1">Desarrollé una plataforma web para generar facturas y cotizaciones. La aplicación maneja distintos parámetros que permiten asignar y/o modificar la información correspondiente a las facturas que asigna la DIAN, permite imprimir las facturas generando un archivo PDF, permite recibir mensajes de las personas interesadas en los servicios ofrecidos por el cliente (a través de un formulario ubicado en la página web del cliente). <br>

                    El stack usado para esta aplicación fue: Flask Web Framework, JavaScript, HTML,
                    CSS, Bootstrap, MySQL, NGINX.
                    </li>
                  </ul>
                </div>
              </div>
              <!-- Grexco -->
              <div>
                <h4 class="font-weight-bold">Auxiliar de Soporte - Grexco S.A.S</h4>
                <span class="text-secondary">Agosto de 2016 - Octubre de 2018</span>
                <ul class="mt-1">
                  <li class="p-1">Brindé soporte telefónico y presencial a los diferentes clientes sobre el
                  software (ERP) ofrecido por la empresa para satisfacer las necesidades del cliente.
                  </li>
                  <li class="p-1">
                  Debía analizar los diferentes incidente reportados para diseñar la mejor solución.
                  </li>
                  <li class="p-1">
                  Propuse y desarrollé una plataforma web de mesa de ayuda que le permitía a los clientes reportar los incidentes que tuvieran. Del lado de la empresa permitía: clasificar los incidentes, tener un mejor control de los tiempos de respuesta y una distribución mas eficiente de las actualizaciones hechas a los aplicativos.
                  <br>
                  El stack usado para este desarrollo fue: Django, MySQL, NGINX, JQuery, Bootstrap.
                  </li>
                </ul>
              </div>
            </div>
            <br>
            <!-- Skills -->
            <div>
              <h4 class="font-weight-bold">He trabajado con:</h4>
              <div class="row mt-sm-3 mt-3">
                <div class="col-sm">
                  <ul class="list-group list-group-flush">
                    <li class="list-group-item p-1">Python</li>
                    <li class="list-group-item p-1">Django</li>
                    <li class="list-group-item p-1">Rest Framework</li>
                    <li class="list-group-item p-1">Flask</li>
                    <li class="list-group-item p-1">SQL</li>
                    <li class="list-group-item p-1">HTML</li>
                    <li class="list-group-item p-1">API Rest</li>
                  </ul>
                </div>
                <div class="col-sm">
                  <ul class="list-group list-group-flush">
                    <li class="list-group-item p-1">Git</li>
                    <li class="list-group-item p-1">Docker</li>
                    <li class="list-group-item p-1">Linux</li>
                    <li class="list-group-item p-1">Oracle</li>
                    <li class="list-group-item p-1">SQLServer</li>
                    <li class="list-group-item p-1">CSS</li>
                    <li class="list-group-item p-1">JQuery</li>
                  </ul>
                </div>
              </div>
            </div>
            <br>
            <!-- Education -->
            <div class="mt-3">
              <h3 class="font-weight-bold mb-4">Educación</h3>
              <!-- Lifelong Learner -->
              <div class="mt-3">
                <h4 class="font-weight-bold">Lifelong Learner</h4>
                <span class="text-secondary">All life</span>
                <div>
                  <ul>
                    <li>Soy autodidacta y me encanta aprender cosas nueva todos los días.</li>
                  </ul>
                </div>
              </div>
              <!-- Platzi -->
              <div class="mt-3">
                <h4 class="font-weight-bold">Cursos</h4>
                <p>He realizado varios cursos online para apoyar mi crecimiento profesional en
                  <a class="text-primary" href="https://platzi.com">Platzi</a>:                
                </p>
                <div class="pl-1 pl-sm-4">
                  <ul class="list-group list-group-flush">
                    <li class="list-group-item p-1">Fundamentos de Docker
                      <span class="text-secondary">Febrero -  2020</span>
                    </li>
                    <li class="list-group-item p-1">Scrum
                      <span class="text-secondary">Enero - 2020</span>
                    </li>
                    <li class="list-group-item p-1">Desarrollo Backend con Python y Django
                      <span class="text-secondary">Noviembre - 2019</span>
                    </li>
                    <li class="list-group-item p-1">Flask
                      <span class="text-secondary">Noviembre - 2019</span>
                    </li>
                    <li class="list-group-item p-1">Fundamentos de Bases de Datos
                      <span class="text-secondary">Noviembre - 2019</span>
                    </li>
                    <li class="list-group-item p-1">Introducción a la Seguridad Informática
                      <span class="text-secondary">Septiembre - 2019</span>
                    </li>
                    <li class="list-group-item p-1">API Rest
                      <span class="text-secondary">Agosto - 2019</span>
                    </li>
                    <li class="list-group-item p-1">Programación Orientada a Objetos
                      <span class="text-secondary">Julio - 2019</span>
                    </li>
                    <li class="list-group-item p-1">Redes de Internet
                      <span class="text-secondary">Julio - 2019</span>
                    </li>
                    <li class="list-group-item p-1">Git y GitHub
                      <span class="text-secondary">Julio - 2019</span>
                    </li>
                    <li class="list-group-item p-1">Pensamiento Lógico
                      <span class="text-secondary">Junio - 2019</span>
                    </li>
                    <li class="list-group-item p-1">Terminal y Línea de Comandos
                      <span class="text-secondary">Mayo - 2019</span>
                    </li>
                    <li class="list-group-item p-1">Ingeniería de Datos con Python
                      <span class="text-secondary">Abril - 2019</span>
                    </li>
                    <li class="list-group-item p-1">Fundamentos de Ingeniería de Software
                      <span class="text-secondary">Abril - 2019</span>
                    </li>
                    <li class="list-group-item p-1">Introducción al Pensamiento Computacional
                      <span class="text-secondary">Febrero - 2019</span>
                    </li>
                  </ul>  
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </main>
    <HmFooter/>
  </div>
</template>
<script>
import HmHeader from '@/components/home/HmHeader'
import HmFooter from '@/components/home/HmFooter'

export default {
  name: 'About',
  components: {
    HmHeader,
    HmFooter
  }
}
</script>